...
const SignIn = () => {
  ...
  return (
    <KeyboardAvoidingView behavior='padding' keyboardVerticalOffset={0} className='h-full bg-black'>
      <ImageBackground source={image.signInBkg} className="h-full w-full flex-col-reverse" resizeMode='cover'>
        <SafeAreaView className='px-3 flex items-center'>
          <View className='bg-neutral-800 w-full rounded-lg py-6 px-4'>
            <Text className='font-pbold text-3xl text-center text-white mb-4'>Sign In</Text>
            <CustumTextInput
            ...
            <View className='flex flex-row justify-center items-center mt-5'>
              <Text className='text-white font-psemibold text-md'>Don’t have an account? </Text>
              <TouchableOpacity activeOpacity={.7} onPress={() => router.navigate(routes.signUp)}>
                <Text className='text-fuchsia-700 font-psemibold text-md'>Sign up</Text>
              </TouchableOpacity>
            </View>
          </View>
        </SafeAreaView>
